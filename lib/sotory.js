 ...
 * TODO: wrap all elements into single container
 */
...
class Sotory {
  ...
  grabStory() {
    window
      ...
      .then(datas => {
        ...
        if (datas.length) {
          this.image.classList.add('c-sotory__avatar--has-story')
          this.datas = datas
  ...
  setupStoryWithLength(storyLength) {
    ...
    document.body.appendChild(storyTopLevelContainer)
  }
  ...
  renderAvatar() {
    ...
    this.image.style.display = 'none'
    this.image.setAttribute('class', 'c-sotory__avatar')
